...
class Preprocessor_transformer(object):
    ...
    def _parse(self, data_frame, is_test=False):
        X = data_frame[self.config['input_text_column']].apply(Preprocessor_transformer.clean_txt).values
        Y = None
        if not is_test:
    ...
    def nn_vectorization(self, train_x, train_full_x, validate_x, test_x):
        ...
        if pretrained_embedding is not None:
            word2embedding = Preprocessor_transformer.load_vector(pretrained_embedding)
